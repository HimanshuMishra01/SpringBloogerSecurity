package com.himanshu.blogger.controller;
...
@Controller
@RequestMapping("/")
@SessionAttributes("blogRoleList")
public class UserController {
	...
	public ModelAndView listUsers(ModelMap model) {
		...
		logger.info("success present : "+model.get("success"));
	...
	/**
	 * This method will provide the medium to add a new user. By GET method it
	 * will identify that it has to load registration page
	 */
	...
	public String saveUser(@Valid User user, BindingResult result, ModelMap model, RedirectAttributes redirectAttributes) {

		System.out.println("save user : " + user);
		if (result.hasErrors()) {
			System.out.println("save error "+result);
			return "registration";
		}
		 ...
		 * 
		 */
		if (!userService.isUniqueUserId(user.getUserId())) {
			FieldError userIdError = new FieldError("user", "userId", messageSource.getMessage("non.unique.ssoId",
					new String[] { user.getUserId() }, Locale.getDefault()));
			System.out.println("save 1 error encounterd");
		...
		redirectAttributes.addFlashAttribute("success","User " + user.getFirstName() + " " + user.getLastName() + " registered successfully");
		model.addAttribute("currentUser", getPrincipal());
		// return "success";
	...
	private String getPrincipal() {
		...
		System.out.println("get principal user = " + userName);
		return userName;
	}
